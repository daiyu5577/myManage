...
export default {
     ...
     */
    setData (key: any, value: any, session = false) {
      const store = session ? 'sessionStorage' : 'localStorage'
      value = JSON.stringify(value)
      this.isSupported && window[store] && (window[store][key] = value)
    },
    getData (key: any, session = false) {
      const store = session ? 'sessionStorage' : 'localStorage'
    ...
    removeData (key: any, session = false) {
      const store = session ? 'sessionStorage' : 'localStorage'
      this.isSupported &&
        window[store] &&
        window[store][key] &&
     ...
     * @param  {Boolean}  session
     */
    addItem (key: any, value: any, index: any, session = false) {
      const store = session ? 'sessionStorage' : 'localStorage'
     ...
     * @param  {Boolean} session
     */
    getItem (key: any, index: any, session = false) {
     ...
     */
    setCookie (key: any, value: any, day: any) {
      const exp: any = new Date(Date.now() + 24 * day * 60 * 60 * 1000)
